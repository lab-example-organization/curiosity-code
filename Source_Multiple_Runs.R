...
conv_outputToFolderName <- function(string, normal_output = TRUE, single = TRUE, number_of_runs) { # takes strings of the form "storing data packet 100 at 2018-10-09 01:55:51" from console output, and outputs a folder name "2018-10-09-010315-GMT-variable-store"
  if(normal_output == TRUE) {
    ...
    if(single == TRUE) {
      ...
      for(runds in 1:number_of_runs) {
        runNames[runds] <- string[nrow(fn_doc_lines)-(length_of_single_run-1)-((number_of_runs-runds)*length_of_single_run)]
      }
      output <- paste0()
    }
  }
