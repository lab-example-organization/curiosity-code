...
syll_learn <- function(P, context = 2){ # context decides whether the learning is vertical (2) or oblique (1)
  ...
  for(population in 1 : P$num_pop) {
    for(number_renewed in 1 : P$nropsp) {
      # Make the reference objects for the teacher - the indices for the syllables the teacher has in his repertoire, and a set of probabilities for each syllable to be learned
      ...
      if(context == 2) { # clear the sylreps rows about to be filled in :D
        for(sex in 1 : 2) {
          for(syllable in 1 : P$sylnum) {
            sylreps[P$offspring_calls[sex, population, number_renewed], syllable, population] <- 0
          }
        }
      } else { # Oblique Learning
          # anything special happen during oblique learning? aside from only males learning and modified learning capabilities?
      }
      for (sex in 1:context) {
        for (sylls_to_learn in 1 : length(source_of_ONEs)) {
          if(probs[sylls_to_learn] <= (P$learnprob[context])) {
            sylreps[P$offspring_calls[sex, population, number_renewed], source_of_ONEs[sylls_to_learn], population] <- 1 # nropsp!!! come on! still have to figure that one out i guess
            ...
            sylreps[P$offspring_calls[sex, population, number_renewed], floor(r_norm), population] <- 1
          }
        }
      }
    }
...
variable.archive <- function(P, timestep) {
  for(number_renewed in 1:P$nropsp) {
    for(population in 1 : P$num_pop) {
      ...
      day.tuh[["curity_mean_t"]][4, (population + ((number_renewed-1) * P$num_pop)), timestep] <- P$pairing.pool[1, 2, population, number_renewed]
      day.tuh[["curity_mean_t"]][5, (population + ((number_renewed-1) * P$num_pop)), timestep] <- P$pairing.pool[2, 2, population, number_renewed]
      day.tuh[["curity_mean_t"]][6, population, timestep] <- P$curiosity_level[P$offspring_calls[1, population, number_renewed], population]
      day.tuh[["curity_mean_t"]][7, population, timestep] <- P$curiosity_level[P$offspring_calls[2, population, number_renewed], population]
      ...
      for(sex in 1:2) {
        ...
        day.tuh[["curity_mean_t"]][sex, population, timestep] <- mean(P$curiosity_level[((1 + ((sex-1) * P$pop_size/2)):(sex * P$pop_size/2)), population])
        day.tuh[["curity_repert"]][(sex + ((population - 1) * 2)), , timestep] <- hist(P$curiosity_level[((1 + ((sex-1) * P$pop_size / 2)):(sex * P$pop_size / 2)), population], breaks = P$curiositybreaks, plot = FALSE)$counts
      }
    }
    return(day.tuh)
  }
...
make.offspring.calls <- function(P, no.parent.turnover = TRUE){
  ...
  for(population in 1:P$num_pop){
    for(sex in 1:2){
      variable_name <- c(sample(P$pop_calls_matrix[sex, ], P$nropsp, replace=F))
      if(no.parent.turnover == TRUE) {
        for(number_renewed in 1:P$nropsp) {
          while(variable_name[number_renewed] %in% P$pairing.pool[sex, , population]) {
            variable_name[number_renewed] <- c(sample(P$pop_calls_matrix[sex, ], 1, replace=F))
          }
        }
      }
      P$offspring_calls[sex, population, ] <-  variable_name
    }
  }
  return(P$offspring_calls)
}
      ...
      for(k in 1 : num_select_chances[context]) {
        if(k == num_select_chances[context]) {
          ...
          for(sex in 1:context) {
            ...
            P$pairing.pool[sex, 2, population, number_renewed] <- P$curiosity_level[auto.teachers[sex], population]
          }
        ...
        selector_context <- list(
          tutor_selector = paste0("selector.index <- P$offspring_calls[1, ", population, ", ", number_renewed, "]"),
        ...
        singer <- ((sort(similarity_golf.score, index.return = TRUE))$ix)[round(P$curiosity_level[selector.index, population] * (P$num_one.pop_singers_sampled[context] * P$num_pop) + 0.5)]
        ...
        if(singer %in% ( (1 + ((population - 1) * P$num_one.pop_singers_sampled[context])) : (population * P$num_one.pop_singers_sampled[context]) ) == TRUE ) {     # ((((population-1)*num_one.pop_singers_sampled)+1):(population*num_one.pop_singers_sampled))
          ...
          for(sex in 1:context) {
            ...
            P$pairing.pool[sex, 2, population, number_renewed] <- P$curiosity_level[indices[sex], population]
          }
          P$pairing.pool[2, 3, population, number_renewed] <- k
          stop = TRUE
          break
  ...
  for(number_renewed in 1 : P$nropsp) {
    for(population in 1 : (P$num_pop)) {
      ...
      for(sex in 1:2) {
        ...
        some.int <- 1
        while((new.curiosity <= 0 | new.curiosity >= 1) && (some.int <= P$new.cur.threshold)) {
        ...
        day.tuh$curity_mean_t[(sex+7),(population + ((number_renewed-1) * P$num_pop)),timestep] <- P$curiosity_level[P$offspring_calls[sex, population, number_renewed], population]
        P$curiosity_level[P$offspring_calls[sex, population, number_renewed], population] <- new.curiosity
        day.tuh$curity_mean_t[10,population,timestep] <- some.int
      }
    }
  }
  return(P$curiosity_level)
}
