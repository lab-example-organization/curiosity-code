      ...
      if(context == 2) { # clear the sylreps rows about to be filled in :D
        for(sex in 1 : 2) {
          for(syllable in 1 : P$sylnum) {
            sylreps[P$pairing.pool[(sex + 2), 1, population, number_renewed], syllable, population] <- 0
          }
        }
      ...
      for (sex in 1:context) {
        for (sylls_to_learn in 1 : length(source_of_ONEs)) {
          if(probs[sylls_to_learn] <= (P$learnprob[context])) {
            sylreps[P$pairing.pool[(sex + 2), 1, population, number_renewed], source_of_ONEs[sylls_to_learn], population] <- 1 # nropsp!!! come on! still have to figure that one out i guess
            ...
            sylreps[P$pairing.pool[(sex + 2), 1, population, number_renewed], floor(r_norm), population] <- 1
          }
        }
      }
    }
  ...
  if(context == 1) {
    for(number_renewed in 1:P$nropsp) {
      for(population in 1 : P$num_pop) {
        ...
        day.tuh[["curity_mean_t"]][6, population, timestep] <- P$pairing.pool[1, 1, population, number_renewed]
        day.tuh[["curity_mean_t"]][7, population, timestep] <- P$pairing.pool[2, 1, population, number_renewed]
    ...
    for(number_renewed in 1:P$nropsp) {
      for(population in 1 : P$num_pop) {
        for(sex in 1:2) {
          ...
          day.tuh[["curity_mean_t"]][(sex + 7), (population + ((number_renewed-1) * P$num_pop)), timestep] <- P$curiosity_level[replaced_index, population]
        }
      }
    }
  }
...
reintegrate.offspring.info <- function() {
  day.tuh[["curity_mean_t"]][] <- pairing.pool[]
  day.tuh[["curity_mean_t"]][] <- pairing.pool[]
