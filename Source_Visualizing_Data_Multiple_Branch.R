...
convert_stored_data <- function(P = P, num_timechunks=thousand_timesteps) {
  ...
  for(data_subset in 1:4) {
    data1s <- paste0(names[data_subset], "_", 1:num_timechunks, " <- readRDS(file = paste0(\"variable-store-\", ", 1:num_timechunks, ", \"-", names[data_subset], ".RData\"))")
...
paste_split_data_runs <- function(data_subset, num_runs = 10, also_mean = TRUE) {
  if(also_mean == TRUE) {
    ...
    pasted_runs <- array(0, c(dim(data_subset), num_runs))
    thing <- paste0("pasted_runs[", 1 : num_runs, "] <- ", quote(data_subset), 1 : num_runs)
    eval(parse(text=thing))
  }
  return(pasted_runs)
}
...
simple_multiplots <- function(R = R, Q = converted_data, simplification_factor = 10, extra_lines = FALSE) {
