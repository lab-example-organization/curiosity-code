...
syll_learn <- function(parameters, moran, select_type = 2, totally_new = FALSE,
   randlearn_context = 1, verbose = FALSE){ 
  ...
  for(population in 1 : parameters$num_pop) {
    ...
    if(select_type == 2) {
      ...
      source_of_ONEs <- which(moran$learning.pool[1, , population] == 1)
      ...
      # double-check tutor isn't out of sylls 
      # before comparing repertoire to pupil.
    ...
    for (sex in 1:select_type) {
      ...
      for (sylls_to_learn in 1:length(source_of_ONEs)) {
        ...
        if(probs[sylls_to_learn] <= (parameters$learnprob[select_type])) {
          moran$learning.pool[
            (sex + 2), source_of_ONEs[sylls_to_learn], population] <- 1
        }
        if(probs[sylls_to_learn] > (
          ...
          if(totally_new == TRUE) {
            ...
            r_norm_pool <- rnorm(100, mean = ifelse(randlearn_context == 1,
                                           ...
                                           source_of_ONEs[sylls_to_learn]),
                                 sd = parameters$stand.dev)
            while(moran$learning.pool[
              (sex + 2), floor(r_norm), population] == 1) {
      ...
      if(totally_new == TRUE) {
        moran$pairing.pool[sex, 5, population] <- mean(
          average_rate_randlearn_overlap)}
    }
  }
  return(moran)
}

variable.archive <- function(parameters, moran, syllable_object, 
                              curiosity_object, data_container, timestep) {
  #context_name <- c("parents&offspring","replacedindividuals")
  #if(context == 1) {
  for (population in 1 : parameters$num_pop) {
    data_container[["curity_mean_t"]][
      3, population, timestep] <- moran$pairing.pool[2, 3, population]
    data_container[["curity_mean_t"]][
      10, population, timestep] <- moran$pairing.pool[3, 3, population]
    
    for (sex in 1:2) {
      data_container[["sylrep_rowcol"]][sex, population, timestep] <- mean(
        rowSums(syllable_object[(
          (1 + ((sex - 1) * (parameters$pop_size / 2))) : (
            sex * (parameters$pop_size / 2))
        ), , population]))
      data_container[["sylrep_dstbxn"]][(((
        population - 1) * 2) + sex), , timestep] <- colSums(
        syllable_object[(
          (1 + ((sex - 1) * (parameters$pop_size / 2))) : (
            sex * (parameters$pop_size / 2))), , population])
        # data_container[sylnums filled in by population[sex]] <- colSums(
          # sylnums called by population[sex])
      data_container[["curity_repert"]][
        (sex + ((population - 1) * 2)), , timestep] <- hist( curiosity_object[
          ((1 + ((sex-1) * parameters$pop_size / 2)):(
            sex * parameters$pop_size / 2)), population
        ], breaks = parameters$curiositybreaks, plot = FALSE)$counts

      data_container[["curity_mean_t"]][sex, population, timestep] <- mean(
        curiosity_object[
          ((1 + ((sex-1) * parameters$pop_size/2)):(
            sex * parameters$pop_size/2)), population
        ])
      data_container[["curity_mean_t"]][
        (sex + 3), population, timestep] <- moran$pairing.pool[
          sex, 2, population]
      data_container[["curity_mean_t"]][
        (sex + 5), population, timestep] <- moran$pairing.pool[
          (sex + 2), 2, population]
      data_container[["curity_mean_t"]][
        (sex + 7), population, timestep] <- moran$pairing.pool[
          (sex + 2), 4, population]
      data_container[["curity_mean_t"]][
        11, population, timestep] <- moran$pairing.pool[
          (sex + 2), 5, population]
      data_container[["curity_mean_t"]][
        12, population, timestep] <- moran$pairing.pool[
          sex, 5, population]
    }
  }
...
update_selexn_data <- function(main_parameters, moran, suitor_choices, 
                               preferred_bird, selector_bird, curiosity_value,
                               selector_population, selection_context, 
                               sylreps_choices, sylrep_selector, 
  ...
  for(bird in 1:selection_context) {
    ...
    moran$pairing.pool[
      pool.row, 1, selector_population] <- selected_pair[bird]
    moran$learning.pool[
      pool.row, , selector_population] <- sylrep_pairs[bird,]
    moran$pairing.pool[
      pool.row, 2, selector_population] <- curiosities[bird]
  }
  moran$pairing.pool[
    (4 - selection_context), 3, selector_population] <- selection_count
  return(moran)
}
...
sing.selection <- function(parameters, moran, curiosity_level, 
                            select_type, sylrep_object, 
  ...
  for(population in 1 : parameters$num_pop) { #population <- 1 rm(population)
    ...
    while(chance_for_selection <= num_select_chances[select_type]) {
      ...
      if(chance_for_selection == num_select_chances[select_type]) {
        ...
        for(MTsylrep_filter in 1:sylrep_fill_chances){
          ...
          if((
            ...
            moran = update_selexn_data(parameters, moran, 
                auto.teachers[1,], MTsylrep_filter, 
                auto.teachers[2,MTsylrep_filter], curiosity_level, 
                population, select_type, sylrep_object[auto.teachers[1,],,], 
                sylrep_object[auto.teachers[2,MTsylrep_filter],,population], 
                num_select_chances[select_type], T)
            
            # should probably fill in some spots in moran$pairing.pool
            # with MTsylrep_filter, 
            # provided the value exceeds 1.
      ...
      if(select_type == 1) {
        ...
        singSuccessFilter <- 1 : ((parameters$one_pop_singers[
          select_type]) * (parameters$num_pop)) # "1-20"
      ...
      if(!interbreed) {
        ...
        if(singer %in% singSuccessFilter) {
          ...
          moran = update_selexn_data(parameters, moran, selection.index, 
        ...
        if(should_continue) {
          for(neighbor in c(1, -1)) {
            if(should_pick_neighbor(neighbor,num_select_chances,select_type,
              ...
              moran = update_selexn_data(parameters, moran, selection.index, 
                                         singer, selector.index, 
                                         curiosity_level, population, 
                                         select_type, selection.sylreps, 
                                         selector.sylrep, 
          ...
          for(neighbor in c(1, -1, 2, -2)) {
            if(should_pick_neighbor(neighbor,num_select_chances,select_type,
              ...
              moran = update_selexn_data(parameters, moran, selection.index, 
                                         ...
                                         curiosity_level, population, 
                                         select_type, selection.sylreps, 
                                         selector.sylrep, chance_for_selection,
                                         F)
        ...
        if(sum(sylrep_object[selection.index[singer], , population]) != 0) {
          ...
          moran = update_selexn_data(parameters, moran, selection.index, 
                                     ...
                                     selection.sylreps, selector.sylrep, 
                                     chance_for_selection, F)
...
curiosity_learn <- function(parameters, moran, 
  ...
  for(population in 1 : (parameters$num_pop)) {
    ...
    for(sex in 1:2) {
      if(moran$pairing.pool[curinh_patterns[inheritance_pattern,sex], 2, 
      population] == 0) {
        stop("probably not the best time to be learning 
        curiosity from your parents right now...")}
      
      curinh_attempts <- 1
      while((moran$pairing.pool[curinh_patterns[inheritance_pattern,sex], 
      2, population] + ((1 - parameters$curlearnprob) * (newcuriosity[
        sex, population]))) < 0) {
      ...
      new.curiosity <- moran$pairing.pool[curinh_patterns[inheritance_pattern,
        sex], 2, population] + ((1 - parameters$curlearnprob) * (newcuriosity[
        sex, population])) # Adding small proportion of noise
      
      moran$pairing.pool[(sex + 2), 4, population] <- moran$pairing.pool[
        (sex + 2), 2, population]
...
recuriosity.offspring <- function(parameters, moran, curiosity_object) {
  for(population in 1:parameters$num_pop) {
    for(sex in 1:2) {
      ...
      curiosity_object[
        moran$pairing.pool[(sex + 2), 1, population], population
        ] <- moran$pairing.pool[(sex + 2), 2, population]
    }
  }
...
resylreps.offspring <- function(parameters, moran, sylrep_object) {
  for(population in 1:parameters$num_pop) {
    for(sex in 1:2) {
      ...
      sylrep_object[
        moran$pairing.pool[(sex + 2), 1, population], , population
        ] <- moran$learning.pool[(sex + 2), , population]
    }
  }
...
store_timesteps <- function(parameters, filename = thousand_timesteps, 
                            object_record = day.tuh, saved_stuff, 
                            syll_container, cur_container, 
  ...
  for(deyteh in 1:length(object_record)) {
    file.create(file.path(FolderName, paste0("variable-store-", filename, 
      "-", names(object_record)[[deyteh]], ".RData")))
    objekshun <- object_record[[deyteh]]
    saveRDS(object = objekshun, file = file.path(FolderName, paste0(
      "variable-store-", filename, "-", names(
        object_record)[[deyteh]], ".RData")))
  }
