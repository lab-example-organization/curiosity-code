...
Define.Parameters <- function(num_timesteps, nropsp, num_pop, pop_size, sylnum, nsspl, num_one.pop_singers_sampled, curlearnprob, learnprob, randlearnprob, stand.dev, curflux, new.cur.threshold){
  ...
  offspring_calls <- array(0, c(2, num_pop, nropsp))
  ...
  Parameters <- list(num_timesteps = num_timesteps, 
                     ...
                     offspring_calls = offspring_calls,
...
initialize.curiosity <- function(P, cur.min, cur.max) {
  ...
  for(pop.num in 1:P$num_pop) {
    for(sexes in 1:2) {
      P$curiosity_level[((1 + ((sexes - 1) * (P$pop_size/2))):(sexes * P$pop_size/2)), pop.num] <- runif(P$pop_size / 2, P$zero_to_one_template[cur.min[P$curiosity_counter[sexes, pop.num]]], P$zero_to_one_template[cur.max[P$curiosity_counter[sexes, pop.num]]])
    }
  }
